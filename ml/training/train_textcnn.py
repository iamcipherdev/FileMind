"""Train Model B — TextCNN (PyTorch, from scratch).

Usage:
  python train_textcnn.py --data ml/data/dataset.jsonl --out ml/artifacts/textcnn.pt
"""
...
def main() -> None:
    ...
    ap.add_argument("--out", default="ml/artifacts/textcnn.pt")
    ...
    model = TextCNN(vocab_size=len(vocab), num_classes=13).to(device)
    ...
    torch.save({"state_dict": model.state_dict(), "vocab_size": len(vocab)}, args.out)
